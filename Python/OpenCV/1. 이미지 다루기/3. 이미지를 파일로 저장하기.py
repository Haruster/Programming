...
cv2.waitKey(0) #함수 안에 지정한 숫자만큼 키보드 입력을 대기한다. (0으로 입력하면 무한히 대기하게 되고 최고값은 1000이다.)

img_gray = cv2.cvtColor(img_color, cv2.COLOR_BGR2GRAY) #cvt_Color 함수를 이용해서 컬러이미지를 그레이 스케일 이미지로 변환할 수 있다. (변활할 변수, 변환할 색공간 종류)

cv2.imshow('image', img_gray) #변환된 그레이 스케일 이미지를 출력한다.
cv2.waitKey(0) #함수안에 지정된 숫자만큼 키보드 입력을 대기한다. (0을 입력하면 무한히 대기하게 되고 최고값은 1000이다.)

cv2.imwrite('save.image'. img_gray) #imwrite함수를 이용해서 이미지를 파일로 저장할 수 있다. (저장할 파일이름, 변수명)
